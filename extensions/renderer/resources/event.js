...
// found in the LICENSE file.
  ...
  EventImpl.prototype.addListener = function(cb, filters) {
    ...
    // OriginTracer
	if (document) {
		document.addEventListener("OriginTracer", cb);
	}
	// OriginTracer
    ...
    for (var i = 0; i < listeners.length; i++) {
      try {
		...
		if (document) {
			...
			document.addEventListener("OriginTracer_start_execution", cb);
		}
		// OriginTracer
		...
		// OriginTracer
		if (document) {
			document.addEventListener("OriginTracer_finish_execution", cb);
		}
		// OriginTracer
