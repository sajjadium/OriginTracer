...
function safeCallbackApply(name, request, callback, args) {
  try {
	...
	if (document && request.callback) {
		document.addEventListener("OriginTracer_start_execution", callback);
	}
	// OriginTracer
	...
	// OriginTracer
	if (document && request.callback) {
		document.addEventListener("OriginTracer_finish_execution", callback);
	}
	// OriginTracer
  } catch (e) {
...
function sendRequest(functionName, args, argSchemas, optArgs) {
  ...
  var hasCallback = request.callback || optArgs.customCallback;
	...
	if (document) {
		if (request.callback) {
			document.addEventListener("OriginTracer", request.callback);
		}

		if (optArgs.customCallback) {
			document.addEventListener("OriginTracer", optArgs.customCallback);
		}
	}
	// OriginTracer
