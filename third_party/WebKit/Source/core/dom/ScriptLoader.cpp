...
bool ScriptLoader::prepareScript(const TextPosition& scriptStartPosition, LegacyTypeSupport supportLegacyTypes)
{
	...
	if (elementDocument.m_inclusion != NULL) {
		m_element->document().m_inclusion->add(m_element, NULL, "script", m_element->document().completeURL(client->sourceAttributeValue()));

//		if (client->asyncAttributeValue())
//			m_element->document().m_inclusion->addAttribute(m_element, "async", "");

//		if (client->deferAttributeValue())
//			m_element->document().m_inclusion->addAttribute(m_element, "defer", "");
	}
	// OriginTracer
