...
// OriginTracer
InclusionNode::InclusionNode(int m_id, const void *m_this, const void *m_parent, const void *m_script, const string &m_type, const KURL &m_url) {
	this->m_id = m_id;
	this->m_this = m_this;
...
string getHostname(const KURL &url) {
	if (url.string().startsWith("chrome-extension:"))
		//return string(url.host().utf8().data()) + ".chrome-extension";
		return ".chrome-extension";
	else
		return url.host().utf8().data();
}
...
Inclusion::Inclusion(Document *m_document, const KURL &m_documentUrl) {
	if (m_documentUrl.isEmpty() || (m_document->ownerElement() != NULL && static_cast<HTMLFrameElementBase *>(m_document->ownerElement()) == NULL) || (m_document->url() == blankURL() && m_document->ownerElement() == NULL)) {
		...
		this->m_documentUrl = KURL(ParsedURLString, m_documentUrl.string());

		m_parentDocument = m_document->parentDocument();
		m_parentFrameNode = m_document->ownerElement();

//		stringstream m_ss;
//		m_ss << m_document << " " << m_documentUrl.string().utf8().data() << " " << m_timestamp;

		stringstream m_filename;
//		m_filename << getenv("LOGS_DIR") << "/" << StringHash::hash(String(m_ss.str().data())) << "_debug";
//		m_filename << getenv("LOGS_DIR") << "/debug";

//		debug_file = new fstream(m_filename.str(), fstream::out | fstream::app);
	}
...
//	}
	...
	if (m_parentDocument && m_parentFrameNode && m_parentDocument->m_inclusion) {
		//pthread_mutex_lock(&m_parentDocument->m_inclusion->m_lock);

		labels.insert(m_parentDocument->m_inclusion->m_inclusionNodes[m_parentFrameNode]->labels.begin(), m_parentDocument->m_inclusion->m_inclusionNodes[m_parentFrameNode]->labels.end());
		labels.insert(getHostname(m_parentDocument->m_inclusion->m_inclusionNodes[m_parentFrameNode]->m_url));

		//pthread_mutex_unlock(&m_parentDocument->m_inclusion->m_lock);
	}
...
//	Element* m_element = static_cast<Element *>((void *)m_in->m_this);

//	if (m_in->m_type != "connect" && m_element->parentElement() != NULL)
//		ss << m_element->parentElement();
//	else
//		ss << "(nil)";
...
//	ss << "\t";
	...
	unordered_set<string>::const_iterator it = m_in->labels.begin();

	ss << "\t" << *it;

	++it;

	while (it != m_in->labels.end()) {
		ss << "," << *it;
		++it;
	}
/*
	if (m_in->m_eventListeners.size() > 0) {
		ss << "\t";

		for (set<pair<string, const void *> >::iterator it = m_in->m_eventListeners.begin(); it != m_in->m_eventListeners.end(); ++it)
			if (m_eventListenerScriptMapping.count(make_tuple(m_in->m_this, (*it).first, (*it).second)) > 0)
				ss << (*it).first << "=" << m_eventListenerScriptMapping[make_tuple(m_in->m_this, (*it).first, (*it).second)] << ",";
	}
*/
/*
	if (m_in->m_type != "connect") {
		if (m_element->hasAttribute("id"))
			ss << "\tid=" << m_element->getAttribute("id").utf8().data();

		if (m_element->hasAttribute("class"))
			ss << "\tclass=" << m_element->getAttribute("class").utf8().data();
	}
*/
...
//	*debug_file << "Add Timer\t" << m_documentUrl.string().utf8().data() << "\t" << m_timer << "\t" << m_executingScripts.front()->m_this << "\t" << m_executingScripts.front()->m_url.string().utf8().data() << endl;
...
void Inclusion::removeEvent(const void *m_element, const string &m_eventType, const void *m_listener) {
	...
	if (this->m_document == NULL || m_inclusionNodes.count(m_element) == 0) {
		//pthread_mutex_unlock(&m_lock);
		return;
	}

//	*debug_file << "Add Event\t" << m_documentUrl.string().utf8().data() << "\t" << m_event << "\t" << m_executingScripts.front()->m_this << "\t" << m_executingScripts.front()->m_url.string().utf8().data() << endl;

	m_eventListenerScriptMapping.erase(make_tuple(m_element, m_eventType, m_listener));

	if (m_inclusionNodes.count(m_element) > 0) {
		m_inclusionNodes[m_element]->m_eventListeners.erase(make_pair(m_eventType, m_listener));
		//highlightInjectedElement(m_element);
	}
...
//	*debug_file << "Add Event\t" << m_documentUrl.string().utf8().data() << "\t" << m_element << "\t" << m_eventType << "\t" << m_listener << "\t" << m_executingScripts.front()->m_this << "\t" << m_executingScripts.front()->m_url.string().utf8().data() << endl;

	m_eventListenerScriptMapping[make_tuple(m_element, m_eventType, m_listener)] = m_executingScripts.front()->m_this;

	if (m_inclusionNodes.count(m_element) > 0) {
		m_inclusionNodes[m_element]->m_eventListeners.insert(make_pair(m_eventType, m_listener));
		//highlightInjectedElement(m_element);
	}
...
void Inclusion::highlightInjectedElement(const void *m_element) {
	//pthread_mutex_lock(&m_lock);

	set<string> not_allowed_elements;
	not_allowed_elements.insert("script");
	not_allowed_elements.insert("link");
	not_allowed_elements.insert("source");
	not_allowed_elements.insert("connect");

	if (this->m_document == NULL || m_inclusionNodes.count(m_element) == 0 || m_document->topDocument().url().string().startsWith("chrome-extension") || not_allowed_elements.count(m_inclusionNodes[m_element]->m_type) > 0) {
		//pthread_mutex_unlock(&m_lock);
		return;
	}

	if (m_inclusionNodes[m_element]->labels.count(".chrome-extension") > 0) {
		String current_style = static_cast<Element *>((void *)m_element)->getAttribute(HTMLNames::styleAttr);
		current_style = current_style.replace(";background-color:#FF0000 !important", "") + ";background-color:#FF0000 !important";

		static_cast<Element *>((void *)m_element)->setAttribute(HTMLNames::styleAttr, current_style.utf8().data());
	}
...
//	}
	...
	stringstream m_filename;
	m_filename << getenv("LOGS_DIR") << "/" << getHash(m_ss.str()) << ".log";

	fstream m_logfile(m_filename.str(), fstream::out | fstream::trunc);
	m_logfile << m_timestamp << "\t" << m_document << "\t" << m_documentUrl.string().utf8().data();
...
void Inclusion::remove(const void *m_this) {
	return;
	//pthread_mutex_lock(&m_lock);
...
void Inclusion::add(const void *m_this, const void *m_parent, const string &m_tag) {
	if (m_tag != "script" &&
		m_tag != "img" &&
		m_tag != "embed" &&
		m_tag != "object" &&
		...
		m_tag != "connect" &&
		m_tag != "link")
		add(m_this, m_parent, m_tag, KURL());
}

void Inclusion::add(const void *m_this, const void *m_parent, const string &m_type, const KURL &m_url) {
	//pthread_mutex_lock(&m_lock);
...
//	*debug_file << "Add\t" << m_type << "\t" << m_documentUrl.string().utf8().data() << "\t" << m_this << "\t" << m_url.string().utf8().data() << endl;
	...
	if (m_inclusionNodes.count(m_this) == 0) {
		if (m_executingScripts.size() == 0) {
			...
			if (m_in == NULL) {
				m_in = new InclusionNode(m_inclusionNodes.size(), m_this, m_parent, NULL, m_type, m_url);

				m_in->labels.insert(labels.begin(), labels.end());
				m_in->labels.insert(getHostname(m_documentUrl));

				if (m_in->m_type == "script")
					m_preloadQueue.push_back(m_in);
			}
		} else {
			m_in = new InclusionNode(m_inclusionNodes.size(), m_this, m_parent, m_executingScripts.front()->m_this, m_type, m_url);

			m_in->labels.insert(m_executingScripts.front()->labels.begin(), m_executingScripts.front()->labels.end());
			m_in->labels.insert(getHostname(m_executingScripts.front()->m_url));
		...
		if (m_inclusionNodes.count(m_in->m_script) == 0)
			m_in->m_script = NULL;
	} else {
		...
		if (m_in->m_url != m_url) {
			m_in->m_url = m_url;
			if (m_executingScripts.size() > 0) {
				m_in->m_script = m_executingScripts.front()->m_this;

				m_in->labels.insert(m_executingScripts.front()->labels.begin(), m_executingScripts.front()->labels.end());
				m_in->labels.insert(getHostname(m_executingScripts.front()->m_url));

				highlightInjectedElement(m_in->m_this);
			}
		}
	}
...
//	*debug_file << "Preload\t" << m_documentUrl.string().utf8().data() << "\t" << m_url << endl;

	InclusionNode *rn = new InclusionNode(m_inclusionNodes.size(), NULL, m_executingScripts.front()->m_this, NULL, "", KURL(ParsedURLString, m_url.data()));
	m_executionQueue.push_back(rn);

	rn->labels.insert(m_executingScripts.front()->labels.begin(), m_executingScripts.front()->labels.end());
	rn->labels.insert(getHostname(m_executingScripts.front()->m_url));
...
//	*debug_file << "Start Script\t" << m_documentUrl.string().utf8().data() << "\t" << m_in->m_this << "\t" << m_in->m_url.string().utf8().data() << endl;
...
//	*debug_file << "Finish Script\t" << m_documentUrl.string().utf8().data() << "\t" << m_in->m_this << "\t" << m_in->m_url.string().utf8().data() << endl;
...
//	*debug_file << "Start Timer\t" << m_documentUrl.string().utf8().data() << "\t" << m_timer << "\t" << m_inclusionNodes[m_timers[m_timer]]->m_this << "\t" << m_inclusionNodes[m_timers[m_timer]]->m_url.string().utf8().data() << endl;
...
//	*debug_file << "Finish Timer\t" << m_documentUrl.string().utf8().data() << "\t" << m_timer << "\t" << m_inclusionNodes[m_timers[m_timer]]->m_this << "\t" << m_inclusionNodes[m_timers[m_timer]]->m_url.string().utf8().data() << endl;
...
void Inclusion::startExecutingEvent(const void *m_element, const string &m_eventType, const void *m_listener) {
	...
	if (m_eventListenerScriptMapping.count(make_tuple(m_element, m_eventType, m_listener)) == 0) {
		//pthread_mutex_unlock(&m_lock);
		return;
	}

//	*debug_file << "Start Event\t" << m_documentUrl.string().utf8().data() << "\t" << m_element << "\t" << m_eventType << "\t" << m_listener << "\t" << m_inclusionNodes[m_eventListenerScriptMapping[make_tuple(m_element, m_eventType, m_listener)]]->m_this << "\t" << m_inclusionNodes[m_eventListenerScriptMapping[make_tuple(m_element, m_eventType, m_listener)]]->m_url.string().utf8().data() << endl;

	startExecutingScript(m_eventListenerScriptMapping[make_tuple(m_element, m_eventType, m_listener)]);
...
void Inclusion::finishExecutingEvent(const void *m_element, const string &m_eventType, const void *m_listener) {
	...
	finishExecutingScript(m_eventListenerScriptMapping[make_tuple(m_element, m_eventType, m_listener)]);

//	*debug_file << "Finish Event\t" << m_documentUrl.string().utf8().data() << "\t" << m_element << "\t" << m_eventType << "\t" << m_listener << "\t" << m_inclusionNodes[m_eventListenerScriptMapping[make_tuple(m_element, m_eventType, m_listener)]]->m_this << "\t" << m_inclusionNodes[m_eventListenerScriptMapping[make_tuple(m_element, m_eventType, m_listener)]]->m_url.string().utf8().data() << endl;
